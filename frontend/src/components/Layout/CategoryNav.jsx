  ...
  return (
    <nav className={`border-t border-border-color bg-white w-full transition-all duration-250 ${mobileNavOpen ? 'max-h-[500px] border-t' : 'max-h-0 md:max-h-none overflow-hidden md:overflow-visible border-t-0 md:border-t'}`}>
      <div className="max-w-[1280px] mx-auto flex flex-col md:flex-row items-start md:items-center justify-center gap-3.5 md:gap-8 px-4 md:px-6 py-3.5 md:py-3 whitespace-nowrap">
        {/* ВСЕ ТОВАРЫ */}
        <button
          className={`text-[12px] md:text-[11.5px] font-extrabold tracking-[1.5px] uppercase cursor-pointer py-1 relative w-full md:w-auto text-left md:text-center ${!activeCategoryId && !activeBrandId ? 'text-accent-red' : 'text-text-main'}`}
          onClick={() => {
            filterCategory(null);
            closeMobileNav();
          }}
        >
          ...
          {!activeCategoryId && !activeBrandId && (
            <span className="hidden md:block absolute -bottom-[2px] left-0 right-0 h-[2px] bg-accent-red"></span>
          )}
        </button>
        ...
        {flatCategories.map(cat => (
          <button
            key={cat.id}
            className={`text-[12px] md:text-[11.5px] font-extrabold tracking-[1.5px] uppercase cursor-pointer py-1 relative w-full md:w-auto text-left md:text-center ${activeCategoryId === cat.id ? 'text-accent-red' : 'text-text-main'}`}
            onClick={() => {
        ...
        {user && (
          ...
          >
            УВЕДОМЛЕНИЯ
            {unreadNotifCount > 0 && (
              <span className="inline-flex items-center justify-center bg-accent-red text-white text-[9px] font-black min-w-[16px] h-[16px] rounded-full px-1 ml-1.5">
