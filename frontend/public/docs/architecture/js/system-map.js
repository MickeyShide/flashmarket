...
function renderInspector(connection, index) {
  ...
  container.innerHTML = `
    <p class="eyebrow">Connection inspector</p>
    <h3>${escapeHtml(entityLabel(from, connection.from))} → ${escapeHtml(entityLabel(to, connection.to))}</h3>
    ${statusBadge(connection.status)}
    <dl class="inspector-list">
      <div><dt>Protocol</dt><dd>${escapeHtml(connection.protocol)}</dd></div>
      <div><dt>Purpose</dt><dd>${escapeHtml(connection.purpose)}</dd></div>
      <div><dt>Contract</dt><dd>${escapeHtml(contract || "Repository-owned contract")}</dd></div>
      <div><dt>Consistency</dt><dd>${escapeHtml(connection.consistency)}</dd></div>
      <div><dt>Failure behaviour</dt><dd>${escapeHtml(connection.failureBehaviour)}</dd></div>
    </dl>`;
}
...
export function initSystemMap(data, index, { onOpenService }) {
  ...
  if (!map || !controls) return { focusNode() {} };
  ...
  controls.innerHTML = FILTERS.map((filter) => `<button type="button" data-map-filter="${filter.id}" class="${filter.id === "all" ? "is-active" : ""}" aria-pressed="${filter.id === "all"}">${filter.label}</button>`).join("");
  ...
  map.innerHTML = `
    ...
    ${nodes.map((node) => {
      ...
      return `<button type="button" class="map-node map-node--${kind}" style="left:${left}%;top:${top}%" data-map-node="${node.id}" data-status="${node.status}" aria-label="Inspect ${escapeHtml(node.name)}">
  ...
  function applyFilter(filter) {
    ...
    announce(`${FILTERS.find((item) => item.id === filter)?.label || filter} connections shown`);
  ...
  function focusNode(nodeId, { flow = false } = {}) {
    ...
    $$(".map-node", map).forEach((node) => node.classList.toggle("is-flow-active", flow && node.dataset.mapNode === visualId));
  }

  controls.addEventListener("click", (event) => {
    const button = event.target.closest("[data-map-filter]");
    if (button) applyFilter(button.dataset.mapFilter);
  });
  ...
  map.addEventListener("mouseover", (event) => {
    ...
    if (path) showTooltip(event, index.get(path.dataset.connectionId), index);
  });
  ...
  map.addEventListener("mouseout", (event) => {
    if (event.target.closest("[data-map-node]") && !event.relatedTarget?.closest?.("[data-map-node]")) focusRelations(pinnedNode);
    if (event.target.closest("[data-connection-id]")) $("[data-connection-tooltip]").hidden = true;
  });
  ...
  map.addEventListener("focusout", () => focusRelations(pinnedNode));
  map.addEventListener("click", (event) => {
    ...
    if (node) {
      ...
      focusRelations(pinnedNode);
      if (pinnedNode.startsWith("service-")) onOpenService(pinnedNode);
      else announce(`${entityLabel(index.get(pinnedNode), pinnedNode)} selected`);
    }
    const path = event.target.closest("[data-connection-id]");
    if (path) renderInspector(index.get(path.dataset.connectionId), index);
  });

  $("[data-consistency-toggle]")?.addEventListener("click", (event) => {
    const active = event.currentTarget.getAttribute("aria-pressed") !== "true";
    event.currentTarget.setAttribute("aria-pressed", String(active));
    event.currentTarget.textContent = active ? "Hide consistency boundaries" : "Show consistency boundaries";
    map.classList.toggle("is-consistency", active);
  });

  applyFilter(activeFilter);
  return { focusNode, applyFilter };
}
