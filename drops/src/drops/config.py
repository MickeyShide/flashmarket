...
class Settings(BaseSettings):
    """Application settings for Drops service."""
    ...
    app_name: str = "FlashMarket Drops"
    ...
    def validate_production_settings(self) -> "Settings":
        ...
        errors: list[str] = []
        if self.debug:
            errors.append("DROPS_DEBUG must be false")
        if self.docs_enabled:
            errors.append("DROPS_DOCS_ENABLED must be false")
        ...
        if "localhost" in self.database_url or "127.0.0.1" in self.database_url:
            errors.append("DROPS_DATABASE_URL must point to production database")
        db_is_internal = self.allow_insecure_internal_services
        if "sslmode" not in self.database_url and not db_is_internal:
            errors.append("DROPS_DATABASE_URL must use TLS (sslmode)")
        if errors:
