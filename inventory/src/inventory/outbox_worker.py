...
from inventory.config import get_settings, utc_now
from inventory.domain.entities import InventoryEventType
...
EVENT_ROUTING_KEYS: dict[str, str] = {
    InventoryEventType.INVENTORY_RESERVED: "inventory.InventoryReserved",
    InventoryEventType.INVENTORY_COMMITTED: "inventory.InventoryCommitted",
    InventoryEventType.RESERVATION_RELEASED: "inventory.ReservationReleased",
}
ROUTED_EVENTS = {"inventory.ReservationReleased"}
    ...
    for _ in range(settings.outbox_batch_size):
        ...
        try:
            ...
            await publish_confirmed(
                ...
                mandatory=routing_key in ROUTED_EVENTS,
            )
...
async def run_connected_worker() -> None:
    ...
    async with connection:
        ...
        while True:
            ...
            touch_heartbeat("/tmp/flashmarket-heartbeat.json", "inventory_outbox")
            await asyncio.sleep(settings.outbox_poll_interval_seconds)
